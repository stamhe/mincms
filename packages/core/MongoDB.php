...
class MongoDB{ 
	...
	{ 
		...
		{ 
			$this->_config_data = params('default');
		}  
	...
	public function switch_db($database = '')
	{
		if (empty($database))
		{
			throw new \Exception('To switch MongoDB databases, a new database name must be specified', 500);
		}
		...
		{
			throw new \Exception('Unable to switch Mongo Databases: ' . $exception->getMessage(), 500);
		}
	}
	...
	*/
	public function drop_db($database = '')
	{
		...
		else
		{
			...
			{
				throw new \Exception('Unable to drop Mongo database `' . $database . '`: ' . $exception->getMessage(), 500);
			}
	...
	public function drop_collection($database = '', $collection = '')
	{
		if (empty($database))
		{
			throw new \Exception('Failed to drop MongoDB collection because database name is empty', 500);
		}
	 ...
	 * @param boolean $enable_end_wildcard   If set to anything other than TRUE, an ending line character "$" will be applicationended to the search value, representing only searching for a value at the end of a line.
	 *
	 * @access public
	 * @return object
	 */
	...
	*/
	public function count($collection = '')
	{
		if (empty($collection))
		{
			throw new \Exception('In order to retrieve a count of documents from MongoDB, a collection name must be passed', 500);
		}
	...
	public function insert($collection = '', $insert = array(), $options = array())
	{
		...
		{
			throw new \Exception('Insert of data into MongoDB failed: ' .$exception->getMessage(), 500);
		}
	}
	...
	public function update($collection = '', $options = array())
	{
		...
		{
			throw new \Exception('Update of data into MongoDB failed: ' . $exception->getMessage(), 500);
		}
	}
	...
	public function delete($collection = '')
	{
		if (empty($collection))
		{
			throw new \Exception('No Mongo collection selected to delete from', 500);
		}
		...
		{
			throw new \Exception('Delete of data into MongoDB failed: ' . $exception->getMessage(), 500);
		}
	}
	...
	public function command($query = array())
	{
		...
		{
			throw new \Exception('MongoDB command failed to execute: ' . $exception->getMessage(), 500);
		}
	}
	...
	public function add_index($collection = '', $fields = array(), $options = array())
	{
		...
		{
			throw new \Exception('An error occurred when trying to add an index to MongoDB Collection', 500);
		}
	}
	...
	public function remove_index($collection = '', $keys = array())
	{
		...
		{
			throw new \Exception('Index could not be removed from MongoDB Collection because no keys were specified', 500);
		}
		...
		{
			throw new \Exception('An error occurred when trying to remove an index from MongoDB Collection', 500);
		}
	}
	...
	public function remove_all_indexes($collection = '')
	{
		if (empty($collection))
		{
			throw new \Exception('No Mongo collection specified to remove all indexes from', 500);
		}
	...
	public function get_dbref($object)
	{
		...
		{
			throw new \Exception('To use MongoDBRef::get() ala get_dbref() you must pass a valid reference object', 500);
		}
	...
	public function create_dbref($collection = '', $field = '', $db_name = '')
	{
		...
		{
			throw new \Exception('To use MongoDBRef::create() ala create_dbref() you must pass a valid field id of the object which to link', 500);
		}
	...
	private function _connect()
	{
		...
		catch (MongoConnectionException $exception)
		{
			...
			{
				throw new \Exception('Unable to connect to MongoDB: ' . $exception->getMessage(), 500);
			}
		}
	}
	...
	{		
		...
		{
			throw new \Exception('The database name must be set to connect to MongoDB', 500);
		}
