...
class Group extends \application\core\ActiveRecord 
{
	...
	*/
	function getDelete_ids(){
		$data = static::find()->all();
		if($data){
			$out = \application\core\Arr::model_tree($data,$value='name',$id='id',$pid='pid',$this->id); 
		 	$out[$this->id] = $this->id;
		 	foreach($out as $k=>$v){
		 		$in[] = $k;
		 	}
	...
	*/
	function getGroup_tree(){
		...
		$data = static::find()->all();  
		$out = \application\core\Arr::parentTree($data,$this->pid); 
