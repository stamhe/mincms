...
class SaeTOAuthV2 {
	 ...
	 *  - applicationonweibo	站内应用专用,站内应用不传display参数,并且response_type为token时,默认使用改display.授权后不会返回access_token，只是输出js刷新站内应用父框架
	 * @return array
	 */
	...
	/**
	 * GET wrapplicationwer for oAuthRequest.
	 *
	 * @return mixed
	 */
	...
	/**
	 * POST wreapplicationer for oAuthRequest.
	 *
	 * @return mixed
	 */
	...
	/**
	 * DELTE wrapplicationer for oAuthReqeust.
	 *
	 * @return mixed
	 */
...
class SaeTClientV2
{
	 ...
	 * @param int $base_application 是否只获取当前应用的数据。0为否（所有数据），1为是（仅当前应用），默认为0。
	 * @return array
	 */
	function public_timeline( $page = 1, $count = 50, $base_application = 0 )
	{
		$params = array();
		$params['count'] = intval($count);
		$params['page'] = intval($page);
		$params['base_application'] = intval($base_application);
	 ...
	 * @return array
	 */
	function home_timeline( $page = 1, $count = 50, $since_id = 0, $max_id = 0, $base_application = 0, $feature = 0 )
	{
		$params = array();
		if ($since_id) {
	 ...
	 * @param int $since_id 若指定此参数，则只返回ID比since_id大的微博消息（即比since_id发表时间晚的微博消息）。可选。
	 * @param int $max_id 若指定此参数，则返回ID小于或等于max_id的微博消息。可选。
	 * @param int $base_application 是否基于当前应用来获取数据。1为限制本应用微博，0为不做限制。默认为0。可选。
	 * @param int $feature 微博类型，0全部，1原创，2图片，3视频，4音乐. 返回指定类型的微博信息内容。转为为0。可选。
	 * @return array
	 */
	function friends_timeline( $page = 1, $count = 50, $since_id = 0, $max_id = 0, $base_application = 0, $feature = 0 )
	{
		return $this->home_timeline( $since_id, $max_id, $count, $page, $base_application, $feature);
	}
	 ...
	 * @param int $base_application 是否基于当前应用来获取数据。1为限制本应用微博，0为不做限制。默认为0。
	 * @param int $feature 过滤类型ID，0：全部、1：原创、2：图片、3：视频、4：音乐，默认为0。
	 * @param int $trim_user 返回值中user信息开关，0：返回完整的user信息、1：user字段仅返回uid，默认为0。
	 * @return array
	 */
	function user_timeline_by_id( $uid = NULL , $page = 1 , $count = 50 , $since_id = 0, $max_id = 0, $feature = 0, $trim_user = 0, $base_application = 0)
	{
		$params = array();
		$params['uid']=$uid;
		if ($since_id) {
	 ...
	 * @return array
	 */
	function user_timeline_by_name( $screen_name = NULL , $page = 1 , $count = 50 , $since_id = 0, $max_id = 0, $feature = 0, $trim_user = 0, $base_application = 0 )
	{
		$params = array();
	 ...
	 * @return array
	 */
	function timeline_batch_by_name( $screen_name, $page = 1, $count = 50, $feature = 0, $base_application = 0)
	{
		$params = array();
	 ...
	 * @return array
	 */
	function timeline_batch_by_id( $uids, $page = 1, $count = 50, $feature = 0, $base_application = 0)
	{
		$params = array();
	 ...
	 * @return array
	 */
	function repost_daily( $count = 20, $base_application = 0)
	{
		$params = array();
		$params['count'] = intval($count);
		$params['base_application'] = intval($base_application);
	 ...
	 * @return array
	 */
	function repost_weekly( $count = 20,  $base_application = 0)
	{
		$params = array();
		$params['count'] = intval($count);
		$params['base_application'] = intval($base_application);
	 ...
	 * @return array
	 */
	function comments_daily( $count = 20,  $base_application = 0)
	{
		$params =  array();
		$params['count'] = intval($count);
		$params['base_application'] = intval($base_application);
	 ...
	 * @return array
	 */
	function comments_weekly( $count = 20, $base_application = 0)
	{
		$params =  array();
		$params['count'] = intval($count);
		$params['base_application'] = intval($base_application);
	 ...
	 * 
	 * @param  int $base_application 是否基于当前应用来获取数据。1表示基于当前应用来获取数据，默认为0。可选。
	 * @return array
	 */
	function hourly_trends( $base_application = 0 )
	{
		$params = array();
		$params['base_application'] = $base_application;
	 ...
	 * @return array
	 */
	function daily_trends( $base_application = 0 )
	{
		$params = array();
		$params['base_application'] = $base_application;
	 ...
	 * @return array
	 */
	function weekly_trends( $base_application = 0 )
	{
		$params = array();
		$params['base_application'] = $base_application;
	 ...
	 *  - base_application		int		是否只获取当前应用的数据。0为否（所有数据），1为是（仅当前应用），默认为0。
